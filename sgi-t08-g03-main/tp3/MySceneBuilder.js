...
*/
  ...
  */
    ...
    */
    ...
    /**
     * External function that can be used to create a mesh/group from a node while also retrieving the billboards
     * 
     * @param {*} node 
     */
    createMeshFromNode(node){

        this.billboards = [];

        const mesh = this.createNode(node);

        return {mesh, billboards:this.billboards};
    }
    ...
    createNode(node, parentMaterial= null,parentCastShadow = false, parentReceiveShadow = false){
        ...
        for (let i = 0; i < children.length; i++){
            ...
            if (child.type === "node"){ // if its a node
                ...
                // if its a billboard save it aside
                if (child.subtype == "billboard" && this.billboards){
 ...
 */
    createLOD(lod, material=null, castshadows=null, receiveshadows=null){
